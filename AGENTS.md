...
- Tests must never perform real network requests. Use `DummyRequestHandler` with `ntfy` to capture requests instead of sending them; set its `status` and `body` fields to exercise success and error paths.
